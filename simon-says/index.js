...
async function makeGuess(color) {
  ...
  for (let i = 0; i < currentSequence.length; i++) {
    ...
    if (
      guessedSequence.length >= i + 1 &&
      guessedSequence[i] !== correctColor
    ) {
      await restart();
      return;
    }
  ...
  if (guessedSequence.length < currentSequence.length) return;
...
async function restart() {
  canClick = false;
  const currentScore = currentSequence.length - 1;
  const record = parseInt(localStorage.getItem("current-record") ?? "0");

  if (currentScore > record) {
    localStorage.setItem("current-record", currentScore);
    document.querySelector("#current-record").innerHTML =
      "Recorde: " + (currentSequence.length - 1).toString();
  }
...
(function () {
  ...
  document.querySelector("#current-record").innerHTML =
    "Recorde: " + (localStorage.getItem("current-record") ?? "0");
})();
